...
public final class TestTimeSource extends AbstractLongTimeSource {
    ...
    public final void m1728plusAssignLRDsOJo(long duration) {
        long longDelta = Duration.m1638toLongimpl(duration, getUnit());
        ...
        if ($i$f$isSaturated == 0) {
            ...
            if ((this.reading ^ longDelta) >= 0 && (this.reading ^ newReading) < 0) {
                m1727overflowLRDsOJo(duration);
            }
            this.reading = newReading;
            return;
        }
        long half = Duration.m1595divUwyO8pc(duration, 2);
        long $this$isSaturated$iv = Duration.m1638toLongimpl(half, getUnit());
        if (!((($this$isSaturated$iv - 1) | 1) == Long.MAX_VALUE)) {
            long readingBefore = this.reading;
            try {
                m1728plusAssignLRDsOJo(half);
                m1728plusAssignLRDsOJo(Duration.m1627minusLRDsOJo(duration, half));
                return;
    ...
    private final void m1727overflowLRDsOJo(long duration) {
        throw new IllegalStateException("TestTimeSource will overflow if its reading " + this.reading + DurationUnitKt.shortName(getUnit()) + " is advanced by " + ((Object) Duration.m1641toStringimpl(duration)) + '.');
    }
}
