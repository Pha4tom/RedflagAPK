...
public class SemaphoreAndMutexImpl {
    ...
    */
    ...
    public final boolean addAcquireToQueue(Waiter waiter) {
        ...
        SemaphoreSegment segment = (SemaphoreSegment) SegmentOrClosed.m1843getSegmentimpl(s$iv);
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        while (true) {
            ...
            if (!SegmentOrClosed.m1845isClosedimpl(objFindSegmentInternal)) {
                Segment segmentM1843getSegmentimpl = SegmentOrClosed.m1843getSegmentimpl(objFindSegmentInternal);
                while (true) {
                    Segment segment = (Segment) atomicReferenceFieldUpdater.get(this);
                    z = false;
                    z2 = true;
                    if (segment.id >= segmentM1843getSegmentimpl.id) {
                        z3 = true;
                        break;
                    }
                    if (!segmentM1843getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                        z3 = false;
                        break;
                    }
                    if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(atomicReferenceFieldUpdater, this, segment, segmentM1843getSegmentimpl)) {
                        if (segment.decPointers$kotlinx_coroutines_core()) {
                            segment.remove();
                        }
                        z3 = true;
                    } else if (segmentM1843getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                        segmentM1843getSegmentimpl.remove();
                    }
                }
                if (z3) {
                    break;
        ...
        SemaphoreSegment semaphoreSegment2 = (SemaphoreSegment) SegmentOrClosed.m1843getSegmentimpl(objFindSegmentInternal);
