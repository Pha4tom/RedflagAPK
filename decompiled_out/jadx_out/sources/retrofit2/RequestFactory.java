...
final class RequestFactory {
    ...
    public static final class Builder {
        ...
        private ParameterHandler<?> parseParameter(int i, Type type, Annotation[] annotationArr, boolean z) {
            ...
            if (z) {
                try {
                    if (Utils.getRawType(type) == yo.class) {
        ...
        private ParameterHandler<?> parseParameterAnnotation(int i, Type type, Annotation[] annotationArr, Annotation annotation) {
            ...
            if (annotation instanceof Part) {
                ...
                Headers headersOf = Headers.of("Content-Disposition", bt0.e("form-data; name=\"", strValue5, "\""), MIME.CONTENT_TRANSFER_ENC, part.encoding());
                if (!Iterable.class.isAssignableFrom(rawType8)) {
                    if (!rawType8.isArray()) {
                        if (MultipartBody.Part.class.isAssignableFrom(rawType8)) {
    ...
    public Request create(Object[] objArr) throws IOException {
        ...
        if (length != parameterHandlerArr.length) {
            throw new IllegalArgumentException(bt0.f(s.p("Argument count (", length, ") doesn't match expected count ("), parameterHandlerArr.length, ")"));
        }
